try:
    ...
    def type_enforced_sampled_factory(typ):
        if typ in MULTI_PARAM_FUNCS:
            return type_enforced.Enforcer(iterable_sample_pct=0)(MULTI_PARAM_FUNCS[typ])
    ...
    print(
        f"- The average time taken to validate the data over {REPEATS} runs (ignoring the first run) is measured."
    )
    print("\n## Results Summary")
    print(
